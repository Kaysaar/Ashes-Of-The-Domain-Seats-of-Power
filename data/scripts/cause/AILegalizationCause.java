...
import static com.fs.starfarer.api.impl.campaign.intel.events.HegemonyAICoresActivityCause.IGNORE_COLONY_THRESHOLD;

public class AILegalizationCause extends BaseHostileActivityCause2 {
    public AILegalizationCause(HostileActivityEventIntel intel) {
        super(intel);
    }
    ...
    public TooltipMakerAPI.TooltipCreator getTooltip() {
        return new BaseFactorTooltip() {
            public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                float opad = 10f;
                if(!AILegalizationCrisis.hasPlayerMadeAICoreLegal()){
                    ...
                    tooltip.addPara("By legalizing AI there is no longer way to deter any repercussions",Misc.getNegativeHighlightColor(),0f);
                }
    ...
    public boolean isNegatedByPLMembership() {
        //if (true) return true;
        return PerseanLeagueMembership.isLeagueMember() && getProgress(false) > 0&&!AILegalizationCrisis.hasPlayerMadeAICoreLegal();
    }
...
//		progress = reduced;
    ...
    public static float getAICorePoints(StarSystemAPI system) {
        float total = 0f;
        java.util.List<MarketAPI> markets = Misc.getMarketsInLocation(system, Factions.PLAYER);
